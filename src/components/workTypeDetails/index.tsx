...
export const PlantInstallation: React.FC<WorkTypeDetailsProps> = ({ task: _task, customer }) => {
    ...
    React.useEffect(() => {
        const fetchEmployees = async () => {
            try {
                ...
                // Fetch Technicians
                const techResponse = await fetch('http://localhost:3000/api/employees?role=Technician', {
                ...
                const assistantResponse = await fetch('http://localhost:3000/api/employees?role=Technical Assistant', {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:3000/api/plant-installation-details', {
                method: 'POST',
                headers: {
...
export const createHardCopyindentCreationTask: React.FC<WorkTypeDetailsProps> = ({ task: _task, customer }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:3000/api/additional-documents/${customer.id}/indent`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
...
export const BillGeneration: React.FC<WorkTypeDetailsProps> = ({ task: _task, customer }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:3000/api/additional-documents/${customer.id}/paybill`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
